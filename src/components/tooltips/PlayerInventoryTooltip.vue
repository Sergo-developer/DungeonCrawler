...
<template>
  <div v-if="hoveredItem" class="item-info">
    <div class="item-info-main">
      <div class="item-info-image" :style="{ backgroundImage: `url(${hoveredItem?.image})` }"></div>
      <div>
  ...
  <div v-if="hoveredItem" class="item-info stats">
    <div v-if="hoveredItem?.type.damage">Damage: {{ hoveredItem?.type.damage }}</div>
    <div v-if="hoveredItem?.type.critChance">Crit chance: {{ hoveredItem?.type.critChance }}%</div>
    ...
    <div v-if="hoveredItem?.type.intelligent">Intelligent: {{ hoveredItem?.type.intelligent }}</div>
    ...
    <div style="color: #ea00ff" v-if="hoveredItem?.feature">{{ hoveredItem?.feature }}</div>
  </div>
</template>
